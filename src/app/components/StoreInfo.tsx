...
const StoreInfo = () => {
  ...
  const isOpen = (day >= 1 && day <= 5 && currentTime >= 600 && currentTime < 1140) ||
  ...
  const renderSlides = (
    ...
    // Ширина одного слайда в процентах
    const slideWidthPercent = 100 / count;
    // Общая ширина контейнера с учетом всех слайдов
    const containerWidthPercent = (slides.length / count) * 100;
          ...
          {slides.map((slide, index) => (
            ...
            >
              <img
                src={slide.src}
                alt={slide.alt}
                className={isSmallScreen ? "w-full h-[240px] rounded-lg object-cover" : "w-full h-full rounded-lg object-cover"}
              />
            </div>
          ))}
        </div>

        {/* Индикаторы — для всех слайдов */}
        <div className="absolute bottom-2 left-1/2 transform -translate-x-1/2 flex space-x-1">
          {Array.from({ length: maxSlide + 1 }, (_, index) => (
              <button
                key={index}
                onClick={() => setCurrentSlide(index)}
                className={`w-2 h-2 rounded-full ${index === safeCurrentSlide ? 'bg-blue-500' : 'bg-white'}`}
              ></button>
            ))}
        </div>
      </div>
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-4 py-6">
      <h2 className="text-2xl font-bold text-gray-800 mb-4">Наш магазин</h2>
      ...
      <div className="bg-gray-50 rounded-xl p-4 md:p-6 shadow-sm">
        ...
        <div className="hidden lg:flex gap-6">
          ...
          <div className="w-1/3 flex flex-col justify-between">
            <div>
              <p className="text-gray-800 font-medium mb-2">Витебск, ул. Терешковой 9В</p>

              <div className="relative group mb-3 no-wrap">
                <button className="relative flex items-center text-red-600 hover:text-red-800 text-sm font-medium">
              ...
              <div className="flex items-start mb-3">
                <Image width={12} height={12} alt='Локация' className="mr-2 self-center" src={'/map.svg'} />
                <span className="text-gray-600 text-sm">Петровщина, Малиновка</span>
              </div>

              <div className="flex items-start mb-3">
                <Image width={12} height={12} alt='Телефоны' className="mr-2 self-center" src={'/phone.svg'} />
                <div className="flex flex-col">
                  <span className="text-gray-600 text-sm">+375 33 677-01-66</span>
                  <span className="text-gray-600 text-sm">+375 29 182-01-66</span>
                </div>
              </div>
            </div>
